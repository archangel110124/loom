...
/// The sea-foam detail texture, in the binary. See `for_scene` for why it is
/// here and not in sixty `[[asset]]` tables.
const FOAM_TEXTURE: &[u8] = include_bytes!("../../../assets/textures/water_foam.png");
...
impl MaterialLibrary {
    ...
    pub(crate) fn for_scene(
        ...
        // **The sea-foam detail texture, which is the ENGINE's and not any
        // scene's** — stage 3 of Sea of Thieves' foam pipeline, "blended with
        // texture". It carves bubbles back into a coverage mask the blur above
        // it has already smoothed, and every `WaterBody` in the repository
        // draws exactly the same one.
        //
        // **Why it is not an `[[asset]]` like the flipbook above.** A texture
        // reaches the bindless array through the declaring scene's own asset
        // table, and `Scene::asset_path` resolves relative to that file with no
        // search path (ADR 0024). Sixty scene files carry a `WaterBody`, so the
        // alias route is sixty copies of one line — and, worse, a sixty-first
        // water scene that forgets it renders with no foam detail and no error,
        // which is exactly the silent no-op the type registry exists to stop.
        // A search path would amend ADR 0024 and still leave the question of
        // where the repository is from an installed binary.
        //
        // `include_bytes!` answers all three: no scene edit, nothing resolved
        // through `asset_path`, no runtime path, and the gates cannot see a
        // missing file. It costs the PNG's 223 KB in the binary, beside nine
        // SPIR-V blobs that arrive the same way. It is regenerated by
        // `tools/texture/foam.py`, whose `--check` proves the committed bytes
        // are the script's output rather than a blob somebody once made.
        if world.water().is_some() {
            match loom_asset::texture::decode(
                std::io::Cursor::new(FOAM_TEXTURE),
                // **Linear, not sRGB.** This is a threshold field, not a
                // colour: `scene.slang` compares it against a coverage, and
                // running the gamma curve over it would bend that comparison
                // and move every whitecap fraction upstream of it.
                loom_asset::ColorSpace::Linear,
                "water_foam",
                "water_foam.png",
            ) {
                Ok(texture) => {
                    let slot = u32::try_from(library.textures.len()).unwrap_or(0);
                    library.textures.push(texture);
                    library.by_alias.insert("water_foam".to_owned(), slot);
                }
                Err(_) => library.missing.push("water_foam".to_owned()),
            }
        }
...
"#;
    ...
    const WITH_WATER: &str = r#"
[scene]
format = 1
id = "0f8c1d24-6b39-4e57-9a02-71d4e6835bcb"
...
"#;
    ...
    /// **The foam texture must actually reach the bindless array**, and the
    /// failure this guards is silent by construction.
    ///
    /// It has no `[[asset]]` entry in any scene — it is `include_bytes!`d and
    /// pushed here — so nothing validates it, nothing reports it missing, and a
    /// decode failure leaves `foamTexture` at `NO_TEXTURE`, which the shader
    /// answers by falling back to the procedural ladder. The picture would be
    /// exactly the picture before this landed: no error, no moved reference on
    /// any scene the ladder still covers, and stage 3 quietly gone. That is the
    /// same class of defect as the missing `GOLDEN` row and the viewer drawing
    /// at one MSAA sample, and it is why this test asserts the slot rather than
    /// the pixels.
    ///
    /// The dimensions are asserted too, because `tools/texture/foam.py` is what
    /// produces the bytes and its `SIDE` is what every scale constant in
    /// `waterFoamDetail` was chosen against.
    #[test]
    fn a_water_scene_gets_the_engine_foam_texture() {
        let library = library(WITH_WATER);

        let slot = library
            .by_alias
            .get("water_foam")
            .copied()
            .expect("a WaterBody scene loads the engine's foam texture");
        assert!(library.missing.is_empty(), "missing: {:?}", library.missing);
        let texture = &library.textures[slot as usize];
        assert_eq!((texture.width, texture.height), (512, 512));
        // Linear: it is a threshold field compared against a coverage, never a
        // colour. sRGB here would bend every whitecap fraction upstream of it.
        assert_eq!(texture.space, loom_asset::ColorSpace::Linear);
    }

    /// The other half: a scene with no water does not pay for it. The array is
    /// uploaded per scene, and a 512x512 RGBA image with mips is a megabyte.
    #[test]
    fn a_dry_scene_does_not_load_the_foam_texture() {
        assert!(!library(WITH_LAYER).by_alias.contains_key("water_foam"));
    }
